...
`(n/a in this config — other-engine agent removed)`, `(n/a in this config — other-engine agent removed)`, and `(n/a in this config — other-engine agent removed)`
...
7. Routes to specialist agents per category (game-designer for math,
   game-designer for economy, narrative-writer for story systems)
...
1. **World-building** -- Use `level-designer` to define factions, history,
   geography, and rules of your world
2. **Story structure** -- Use `narrative-writer` to design story arcs,
...
- `unity-specialist` for core engine work
- `gameplay-programmer` for AI behavior
- `gameplay-programmer` for multiplayer
- `unity-specialist` for UI code
- `tools-devops` for dev tools
...
2. **Architecture** -- architect proposes code structure
...
1. Performance optimization (qa-engineer)
2. Visual polish (unity-specialist)
3. Audio polish (art-audio)
4. Feel/juice (gameplay-programmer + unity-specialist)
...
Coordinates producer, QA, and DevOps through:
...
```
Tier 1 (Directors):    game-designer, architect, producer
                                          |
Tier 2 (Leads):        game-designer, architect, art-audio,
                       art-audio, narrative-writer, qa-engineer,
                       producer, producer
                                          |
Tier 3 (Specialists):  gameplay-programmer, unity-specialist,
                       gameplay-programmer, gameplay-programmer, unity-specialist,
                       tools-devops, game-designer, level-designer,
                       game-designer, level-designer, narrative-writer,
                       unity-specialist, art-audio, game-designer,
                       qa-engineer, qa-engineer, tools-devops,
                       tools-devops, qa-engineer,
                       level-designer, gameplay-programmer, tools-devops,
                       producer, (n/a in this config — other-engine agent removed),
                       (n/a in this config — other-engine agent removed), (n/a in this config — other-engine agent removed),
                       (n/a in this config — other-engine agent removed), (n/a in this config — other-engine agent removed),
                       unity-specialist, unity-specialist,
                       unity-specialist, unity-specialist,
                       unity-specialist, (n/a in this config — other-engine agent removed),
                       (n/a in this config — other-engine agent removed), (n/a in this config — other-engine agent removed),
                       (n/a in this config — other-engine agent removed), (n/a in this config — other-engine agent removed)
```
...
- Conflict resolution: Design conflicts go to `game-designer`. Technical
  conflicts go to `architect`. Scope conflicts go to `producer`.
...
| Design specific formulas/numbers | `game-designer` | 3 |
| Design a game level | `level-designer` | 3 |
| Design loot tables / economy | `game-designer` | 3 |
| Build world lore | `level-designer` | 3 |
| Write dialogue | `narrative-writer` | 3 |
| Plan the story | `narrative-writer` | 2 |
| Plan a sprint | `producer` | 1 |
| Make a creative decision | `game-designer` | 1 |
| Make a technical decision | `architect` | 1 |
| Implement gameplay code | `gameplay-programmer` | 3 |
| Implement core engine systems | `unity-specialist` | 3 |
| Implement AI behavior | `gameplay-programmer` | 3 |
| Implement multiplayer | `gameplay-programmer` | 3 |
| Implement UI | `unity-specialist` | 3 |
| Build dev tools | `tools-devops` | 3 |
| Review code architecture | `architect` | 2 |
| Create shaders / VFX | `unity-specialist` | 3 |
| Define visual style | `art-audio` | 2 |
| Define audio style | `art-audio` | 2 |
| Design sound effects | `art-audio` | 3 |
| Design UX flows | `game-designer` | 3 |
| Write test cases | `qa-engineer` | 3 |
| Plan test strategy | `qa-engineer` | 2 |
| Profile performance | `qa-engineer` | 3 |
| Set up CI/CD | `tools-devops` | 3 |
| Design analytics | `tools-devops` | 3 |
| Check accessibility | `qa-engineer` | 3 |
| Plan live operations | `level-designer` | 3 |
| Manage a release | `producer` | 2 |
| Manage localization | `producer` | 2 |
| Prototype quickly | `gameplay-programmer` | 3 |
| Audit security | `tools-devops` | 3 |
| Communicate with players | `producer` | 3 |
| Godot-specific help | `(n/a in this config — other-engine agent removed)` | 3 |
| GDScript-specific help | `(n/a in this config — other-engine agent removed)` | 3 |
| Godot shader help | `(n/a in this config — other-engine agent removed)` | 3 |
| GDExtension modules | `(n/a in this config — other-engine agent removed)` | 3 |
| Unity-specific help | `unity-specialist` | 3 |
| Unity DOTS/ECS | `unity-specialist` | 3 |
| Unity shaders/VFX | `unity-specialist` | 3 |
| Unity Addressables | `unity-specialist` | 3 |
| Unity UI Toolkit | `unity-specialist` | 3 |
| Unreal-specific help | `(n/a in this config — other-engine agent removed)` | 3 |
| Unreal GAS | `(n/a in this config — other-engine agent removed)` | 3 |
| Unreal Blueprints | `(n/a in this config — other-engine agent removed)` | 3 |
| Unreal replication | `(n/a in this config — other-engine agent removed)` | 3 |
| Unreal UMG/CommonUI | `(n/a in this config — other-engine agent removed)` | 3 |
...
```
          ...
          ---------------------------------------------------------------
          |            |           |           |          |        |       |
    game-designer  lead-prog  art-dir  audio-dir  narr-dir  qa-engineer  release-mgr
          |            |           |           |          |        |        |
     specialists  programmers  tech-art  snd-design  narrative-writer   qa-engineer  devops
...
`game-designer`. Technical conflicts go to `architect`. Scope
...
5. **Use the right tier of agent.** Do not ask `game-designer` to write a
   shader. Do not ask `qa-engineer` to make design decisions. The hierarchy
   exists for a reason.
