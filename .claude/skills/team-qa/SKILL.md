---
name: team-qa
description: "Orchestrate the QA team through a full testing cycle. Coordinates qa-engineer (strategy + test plan) and qa-engineer (test case writing + bug reporting) to produce a complete QA package for a sprint or feature. Covers: test plan generation, test case writing, smoke check gate, manual QA execution, and sign-off report."
...
- **qa-engineer** — QA strategy, test plan generation, story classification, sign-off report
- **qa-engineer** — Test case writing, bug report writing, manual QA documentation

## How to Delegate

Use the Task tool to spawn each team member as a subagent:
- `subagent_type: qa-engineer` — Strategy, planning, classification, sign-off
- `subagent_type: qa-engineer` — Test case writing and bug report writing

Always provide full context in each agent's prompt (story file paths, QA plan path, scope constraints). Launch independent qa-engineer tasks in parallel where possible (e.g., multiple stories in Phase 5 can be scaffolded simultaneously).
...
### Phase 2: QA Strategy (qa-engineer)

Spawn `qa-engineer` via Task to review all in-scope stories and produce a QA strategy.

Prompt the qa-engineer to:
...
If the smoke check result is **FAIL**, the qa-engineer must list the failures prominently. QA cannot proceed past the strategy phase with a failed smoke check.

Present the qa-engineer's full strategy to the user, then use `AskUserQuestion`:
...
Spawn `qa-engineer` via Task for each story (run in parallel where possible), providing:
- The story file path
...
After each FAIL result: use `AskUserQuestion` to collect the failure description, then spawn `qa-engineer` via Task to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-engineer` via Task to produce the sign-off report using all results from Phases 4–6.
