...
**Director gate skip rule**: Before spawning game-designer, art-audio, or any other Tier 1/2 director for review (outside of PHASE-GATE triggers), apply the resolved mode: skip if solo mode; skip if lean mode and this is not a PHASE-GATE.

## Team Composition
- **game-designer** — User flows, wireframes, accessibility, input handling
- **unity-specialist** — UI framework, screens, widgets, data binding, implementation
- **art-audio** — Visual style, layout polish, consistency with art bible
...
- **qa-engineer** — Audits accessibility compliance at Phase 4
...
- `subagent_type: game-designer` — User flows, wireframes, accessibility, input handling
- `subagent_type: unity-specialist` — UI framework, screens, widgets, data binding
- `subagent_type: art-audio` — Visual style, layout polish, art bible consistency
- `subagent_type: [UI engine specialist]` — Engine-specific UI pattern validation (e.g., unity-specialist, (n/a in this config — other-engine agent removed), (n/a in this config — other-engine agent removed))
- `subagent_type: qa-engineer` — Accessibility compliance audit
...
- (b) Proceed without the pattern library — unity-specialist will treat all patterns created as new and add each to a new `design/ux/interaction-patterns.md` at completion

Do NOT invent or assume patterns from the feature name or GDD alone. If the user chooses (b), explicitly instruct unity-specialist in Phase 3 to treat all patterns as new and document them in `design/ux/interaction-patterns.md` when implementation is complete. Note the pattern library status (created / absent / updated) in the final summary report.

Summarize the context in a brief for the game-designer: what the player is doing, what they need, what constraints apply, and which existing patterns are relevant.

### Phase 1b: UX Spec Authoring

Invoke `/ux-design [feature name]` skill OR delegate directly to game-designer to produce `design/ux/[feature-name].md` following the `ux-spec.md` template.
...
**Gate**: Do not proceed to Phase 2 until the verdict is APPROVED. If the verdict is NEEDS REVISION, the game-designer must address the flagged issues and re-run the review. The user may explicitly accept a NEEDS REVISION risk and proceed, but this must be a conscious decision — present the specific concerns via `AskUserQuestion` before asking whether to proceed.

### Phase 2: Visual Design

Delegate to **art-audio**:
...
- Output: engine UI implementation notes to hand off to unity-specialist before they begin

If no engine is configured, skip this step.

Delegate to **unity-specialist**:
- Implement the UI following the UX spec and visual design spec
- **Use patterns from `design/ux/interaction-patterns.md`** — do not reinvent patterns that are already specified. If a pattern almost fits but needs modification, note the deviation and flag it for game-designer review.
...
- **game-designer**: Verify implementation matches wireframes and interaction spec. Test keyboard-only and gamepad-only navigation. Check accessibility features function correctly.
- **art-audio**: Verify visual consistency with art bible. Check at minimum and maximum supported resolutions.
- **qa-engineer**: Verify compliance against the committed accessibility tier documented in `design/accessibility-requirements.md`. Flag any violations as blockers.
...
delegated to sub-agents and sub-skills (`/ux-design`, `unity-specialist`). Each enforces the
