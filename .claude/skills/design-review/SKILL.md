...
|------------------------|-------------------|
| Costs, prices, drops, rewards, economy | `game-designer` |
| Combat stats, damage, health, DPS | `game-designer`, `game-designer` |
| AI behaviour, pathfinding, targeting | `gameplay-programmer` |
| Level layout, spawning, wave structure | `level-designer` |
| Player progression, XP, unlocks | `game-designer`, `game-designer` |
| UI, HUD, menus, player-facing displays | `game-designer`, `unity-specialist` |
| Dialogue, quests, story, lore | `narrative-writer` |
| Animation, feel, timing, juice | `gameplay-programmer` |
| Multiplayer, sync, replication | `gameplay-programmer` |
| Audio cues, music triggers | `art-audio` |
| Performance, draw calls, memory | `qa-engineer` |
| Engine-specific patterns or APIs | Primary engine specialist (from `.claude/docs/technical-preferences.md`) |
| Acceptance criteria, test coverage | `qa-engineer` |
| Data schema, resource structure | `game-designer` |
...
Spawn `game-designer` for all GDDs that contain formulas or system interaction rules.
...
- **`game-designer`**: For every formula in the GDD, plug in boundary values (minimum and maximum plausible inputs). Report whether any outputs go degenerate — negative values, division by zero, infinity, or nonsensical results at the extremes.

- **`qa-engineer`**: Review every acceptance criterion. Flag any that are not independently testable — phrases like "feels balanced", "works correctly", "performs well" are not ACs. Suggest concrete rewrites for any that fail this test.

### Step 3 — Senior lead review

After all specialists respond, spawn `game-designer` as the **senior reviewer**:
...
- The game-designer's synthesis becomes the **final verdict** in Phase 4.

### Step 4 — Surface disagreements

If specialists disagree with each other or with the game-designer, do NOT silently pick one view. Present the disagreement explicitly in Phase 4 so the user can adjudicate.

Mark every finding with its source: `[game-designer]`, `[game-designer]`, `[game-designer]` etc.
...
Summary: [2-3 sentence summary of key findings from game-designer verdict]
