---
name: team-level
description: "Orchestrate level design team: level-designer + narrative-writer + level-designer + art-audio + game-designer + qa-engineer for complete area/level creation."
...
- `subagent_type: narrative-writer` — Narrative purpose, characters, emotional arc
- `subagent_type: level-designer` — Lore context, environmental storytelling, world rules
- `subagent_type: level-designer` — Spatial layout, pacing, encounters, navigation
- `subagent_type: game-designer` — Enemy compositions, loot tables, difficulty balance
- `subagent_type: art-audio` — Visual theme, color palette, lighting, asset requirements
- `subagent_type: qa-engineer` — Navigation clarity, colorblind safety, cognitive load
- `subagent_type: qa-engineer` — Test cases, boundary testing, playtest checklist
...
### Step 1: Narrative + Visual Direction (narrative-writer + level-designer + art-audio, parallel)

Spawn all three agents simultaneously — issue all three Task calls before waiting for any result.

Spawn the `narrative-writer` agent to:
...
**The art-audio's visual targets from Step 1 must be passed to the level-designer in Step 2** as explicit constraints. Layout decisions happen within the visual direction, not before it.
...
- Narrative brief (from narrative-writer)
- Lore foundation (from level-designer)
- **Visual direction targets (from art-audio)** — layout must work within these targets, not contradict them
...
- Define pacing curve (tension peaks, rest areas, exploration zones) — coordinated with the emotional arc from narrative-writer
- Place encounters with difficulty progression
- Design environmental puzzles or navigation challenges
- Define points of interest and landmarks for wayfinding — these must match the visual landmarks the art-audio specified
...
### Step 3: Systems Integration (game-designer)
Spawn the `game-designer` agent to:
...
### Step 4: Production Concepts + Accessibility (art-audio + qa-engineer, parallel)

**Note**: The art-audio's directional pass (visual theme, color targets, mood) happened in Step 1. This pass is location-specific production concepts — given the finalized layout, what does each specific space look like?

Spawn the `art-audio` agent with the finalized layout from Step 2:
...
Spawn the `qa-engineer` agent in parallel to:
...
**Gate**: Use `AskUserQuestion` to present both Step 4 results. If the qa-engineer returned any BLOCKING concerns, highlight them prominently and offer:
- (a) Return to level-designer and art-audio to redesign the flagged elements before Step 5
