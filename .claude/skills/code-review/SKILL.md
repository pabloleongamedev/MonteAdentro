...
For Logic and Integration stories, also spawn `qa-engineer` via Task in parallel with the engine specialists. Pass:
- The implementation files being reviewed
- The story's `## QA Test Cases` section (the pre-written test specs from qa-engineer)
...
For Visual/Feel and UI stories: qa-engineer reviews whether the manual verification steps in `## QA Test Cases` are achievable with the implementation as written — e.g., "is the state the manual checker needs to reach actually reachable?"
...
[qa-engineer findings: test hooks, coverage gaps, untestable paths, new edge cases]
