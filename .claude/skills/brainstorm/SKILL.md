...
**After pillars and anti-pillars are agreed, spawn BOTH `game-designer` AND `art-audio` via Task in parallel before moving to Phase 5. Issue both Task calls simultaneously — do not wait for one before starting the other.**

- **`game-designer`** — gate **CD-PILLARS** (`.claude/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-audio`** — gate **AD-CONCEPT-VISUAL** (`.claude/docs/director-gates.md`)
...
- Tab **"Pillars"**: present game-designer feedback. Options mirror the standard CD-PILLARS handling — `Lock in as-is` / `Revise [specific pillar]` / `Discuss further`.
- Tab **"Visual anchor"**: present the art-audio's 2-3 named visual direction options. Options: each named direction (one per option) + `Combine elements across directions` + `Describe my own direction`.
...
If the game-designer returns CONCERNS or REJECT on pillars, resolve pillar issues before asking for the visual anchor selection — visual direction should flow from confirmed pillars.
...
**After identifying biggest technical risks, spawn `architect` via Task using gate TD-FEASIBILITY (`.claude/docs/director-gates.md`) before scope tiers are defined.**
...
**Path A — Design-First** (recommended if the concept is well-defined):
   ...
   4. "Discuss vision with the `game-designer` agent for pillar refinement"
