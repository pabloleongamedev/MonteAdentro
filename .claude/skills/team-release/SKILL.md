---
name: team-release
description: "Orchestrate the release team: coordinates producer, qa-engineer, tools-devops, and producer to execute a release from candidate to deployment."
...
## Team Composition
- **producer** — Release branch, versioning, changelog, deployment
- **qa-engineer** — Test sign-off, regression suite, release quality gate
- **tools-devops** — Build pipeline, artifacts, deployment automation
- **tools-devops** — Pre-release security audit (invoke if game has online/multiplayer features or player data)
- **tools-devops** — Verify telemetry events fire correctly and dashboards are live
...
- `subagent_type: producer` — Release branch, versioning, changelog, deployment
- `subagent_type: qa-engineer` — Test sign-off, regression suite, release quality gate
- `subagent_type: tools-devops` — Build pipeline, artifacts, deployment automation
- `subagent_type: tools-devops` — Security audit for online/multiplayer/data features
- `subagent_type: tools-devops` — Telemetry event verification and dashboard readiness
...
- `subagent_type: gameplay-programmer` — Netcode stability sign-off (invoke if game has multiplayer)
...
- **tools-devops**: Build release artifacts for all target platforms. Verify builds are clean and reproducible. Run automated tests in CI.
- **tools-devops** *(if game has online features, multiplayer, or player data)*: Conduct pre-release security audit. Review authentication, anti-cheat, data privacy compliance. Sign off on security posture.
- **gameplay-programmer** *(if game has multiplayer)*: Sign off on netcode stability. Verify lag compensation, reconnect handling, and bandwidth usage under load.

### Phase 4: Localization, Performance, and Analytics
Delegate (can run in parallel with Phase 3 if resources available):
- Verify all strings are translated (delegate to **producer** if available)
- Run performance benchmarks against targets (delegate to **qa-engineer** if available)
- **tools-devops**: Verify all telemetry events fire correctly on release build. Confirm dashboards are receiving data. Check that critical funnels (onboarding, progression, monetization if applicable) are instrumented.
- Output: localization, performance, and analytics sign-off

### Phase 5: Go/No-Go
Delegate to **producer**:
- Collect sign-off from: qa-engineer, producer, tools-devops, tools-devops (if spawned in Phase 3), gameplay-programmer (if spawned in Phase 3), and architect
...
Delegate to **producer** + **tools-devops**:
...
- **producer**: Generate release report (what shipped, what was deferred, metrics)
...
- **tools-devops**: Confirm live dashboards are healthy; alert if any critical events are missing
