...
**Agent delegation (MANDATORY)**: Spawn `game-designer` via Task before finalising edge cases. Provide: the completed Sections C and D, and ask them to identify edge cases from the formula and rule space that the main session may have missed. For narrative systems, also spawn `narrative-writer`. Present their findings and ask the user which to include.
...
**Review mode check** (apply before spawning):
- `solo` → skip this agent spawn. Draft the section without the specialist. Add a note: "`qa-engineer` not consulted — Solo mode. Review manually before production."
- `lean` → skip unless this is a section with HIGH implementation risk (Sections D and H only). For other sections, draft without the agent.
- `full` → spawn as described below.

**Agent delegation (MANDATORY)**: Spawn `qa-engineer` via Task before finalising acceptance criteria. Provide: the completed GDD sections C, D, E, and ask them to validate that the criteria are independently testable and cover all core rules and formulas. Surface any gaps or untestable criteria to the user.
...
For required systems: **spawn `art-audio` via Task** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
For **Visual/Audio** (non-required systems): Coordinate with `art-audio` and `art-audio` if detail is needed. Often a brief note suffices at the GDD stage.
...
For **UI Requirements**: Coordinate with `game-designer` for complex UI systems.
...
Before finalizing the GDD, spawn `game-designer` via Task using gate **CD-GDD-ALIGN** (`.claude/docs/director-gates.md`).
...
|----------------|---------------|---------------------|
| **Foundation/Infrastructure** (event bus, save/load, scene mgmt, service locator) | `game-designer` | `gameplay-programmer` (feasibility), `unity-specialist` (engine integration) |
| Combat, damage, health | `game-designer` | `game-designer` (formulas), `gameplay-programmer` (enemy AI), `art-audio` (hit feedback visual direction, VFX intent) |
...
| Dialogue, quests, lore | `game-designer` | `narrative-writer` (story), `narrative-writer` (content), `art-audio` (character visual profiles, cinematic tone) |
| UI systems (HUD, menus) | `game-designer` | `game-designer` (flows), `unity-specialist` (feasibility), `art-audio` (visual style direction), `unity-specialist` (render/shader constraints) |
| Audio systems | `game-designer` | `art-audio` (direction), `art-audio` (specs) |
| AI, pathfinding, behavior | `game-designer` | `gameplay-programmer` (implementation), `game-designer` (scoring) |
| Level/world systems | `game-designer` | `level-designer` (spatial), `level-designer` (lore) |
| Camera, input, controls | `game-designer` | `game-designer` (feel), `gameplay-programmer` (feasibility) |
| Animation, character movement | `game-designer` | `art-audio` (animation style, pose language), `unity-specialist` (rig/blend constraints), `gameplay-programmer` (feel) |
| Visual effects, particles, shaders | `game-designer` | `art-audio` (VFX visual direction), `unity-specialist` (performance budget, shader complexity), `game-designer` (trigger/state integration) |
| Character systems (stats, archetypes) | `game-designer` | `art-audio` (character visual archetype), `narrative-writer` (character arc alignment), `game-designer` (stat formulas) |
