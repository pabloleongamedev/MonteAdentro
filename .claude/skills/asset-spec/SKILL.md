...
**Mode behavior:**
- `full` (default): spawn both `art-audio` and `unity-specialist` in parallel
- `lean`: spawn `art-audio` only — faster, skips technical constraint pass
...
- **level**: Read `design/levels/[target-name].md`. Extract art requirements, asset list, VFX needs, and the art-audio's production concept specs from Step 4.
...
### Lean mode — spawn art-audio only (skip unity-specialist).

### Solo mode — skip both. Derive specs from art bible rules alone, noting that technical constraints were not validated.

**Collect both responses before Phase 4.** If any conflict exists between art-audio and unity-specialist (e.g., art-audio specifies 4K textures but unity-specialist flags the engine budget requires 512px), surface it explicitly — do NOT silently resolve.
...
2. In `lean` mode or if `unity-specialist` blocks: proceed with art-audio output only — note that technical constraints were not validated
3. In `solo` mode or if `art-audio` blocks: derive descriptions from art bible rules — flag as "Art director not consulted — verify against art bible before production"
