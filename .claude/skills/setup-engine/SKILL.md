...
- **Additional Specialists**: unity-specialist (ECS, Jobs system, Burst compiler), unity-specialist (asset loading, memory management, content catalogs)
...
```markdown
## Engine Specialists
- **Primary**: (n/a in this config — other-engine agent removed)
- **Language/Code Specialist**: (n/a in this config — other-engine agent removed) (Blueprint graphs) or (n/a in this config — other-engine agent removed) (C++)
- **Shader Specialist**: (n/a in this config — other-engine agent removed) (no dedicated shader specialist — primary covers materials)
- **UI Specialist**: (n/a in this config — other-engine agent removed) (UMG widgets, CommonUI, input routing, widget styling)
- **Additional Specialists**: (n/a in this config — other-engine agent removed) (Gameplay Ability System, attributes, gameplay effects), (n/a in this config — other-engine agent removed) (property replication, RPCs, client prediction, netcode)
...
| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Game code (.cpp, .h files) | (n/a in this config — other-engine agent removed) |
| Shader / material files (.usf, .ush, Material assets) | (n/a in this config — other-engine agent removed) |
| UI / screen files (.umg, UMG Widget Blueprints) | (n/a in this config — other-engine agent removed) |
| Scene / prefab / level files (.umap, .uasset) | (n/a in this config — other-engine agent removed) |
| Native extension / plugin files (Plugin .uplugin, modules) | (n/a in this config — other-engine agent removed) |
| Blueprint graphs (.uasset BP classes) | (n/a in this config — other-engine agent removed) |
| General architecture review | (n/a in this config — other-engine agent removed) |
```
...
- When the user chose **GDScript**: copy the GDScript CLAUDE.md template from Appendix A1 exactly. NEVER add "C++ via GDExtension" to the Language field. GDScript projects may use GDExtension, but it is not a primary project language. The `(n/a in this config — other-engine agent removed)` in the routing table is available for when native extensions are needed — it does not make C++ a project language.
...
## Engine Specialists
- **Primary**: (n/a in this config — other-engine agent removed)
- **Language/Code Specialist**: (n/a in this config — other-engine agent removed) (all .gd files)
- **Shader Specialist**: (n/a in this config — other-engine agent removed) (.gdshader files, VisualShader resources)
- **UI Specialist**: (n/a in this config — other-engine agent removed) (no dedicated UI specialist — primary covers all UI)
- **Additional Specialists**: (n/a in this config — other-engine agent removed) (GDExtension / native C++ bindings only)
...
| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Game code (.gd files) | (n/a in this config — other-engine agent removed) |
| Shader / material files (.gdshader, VisualShader) | (n/a in this config — other-engine agent removed) |
| UI / screen files (Control nodes, CanvasLayer) | (n/a in this config — other-engine agent removed) |
| Scene / prefab / level files (.tscn, .tres) | (n/a in this config — other-engine agent removed) |
| Native extension / plugin files (.gdextension, C++) | (n/a in this config — other-engine agent removed) |
| General architecture review | (n/a in this config — other-engine agent removed) |
```
...
## Engine Specialists
- **Primary**: (n/a in this config — other-engine agent removed)
- **Language/Code Specialist**: (n/a in this config — other-engine agent removed) (all .cs files)
- **Shader Specialist**: (n/a in this config — other-engine agent removed) (.gdshader files, VisualShader resources)
- **UI Specialist**: (n/a in this config — other-engine agent removed) (no dedicated UI specialist — primary covers all UI)
- **Additional Specialists**: (n/a in this config — other-engine agent removed) (GDExtension / native C++ bindings only)
...
| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Game code (.cs files) | (n/a in this config — other-engine agent removed) |
| Shader / material files (.gdshader, VisualShader) | (n/a in this config — other-engine agent removed) |
| UI / screen files (Control nodes, CanvasLayer) | (n/a in this config — other-engine agent removed) |
| Scene / prefab / level files (.tscn, .tres) | (n/a in this config — other-engine agent removed) |
| Project config (.csproj, NuGet) | (n/a in this config — other-engine agent removed) |
| Native extension / plugin files (.gdextension, C++) | (n/a in this config — other-engine agent removed) |
| General architecture review | (n/a in this config — other-engine agent removed) |
```
...
## Engine Specialists
- **Primary**: (n/a in this config — other-engine agent removed)
- **GDScript Specialist**: (n/a in this config — other-engine agent removed) (.gd files — gameplay/UI scripts)
- **C# Specialist**: (n/a in this config — other-engine agent removed) (.cs files — performance-critical systems)
- **Shader Specialist**: (n/a in this config — other-engine agent removed) (.gdshader files, VisualShader resources)
- **UI Specialist**: (n/a in this config — other-engine agent removed) (no dedicated UI specialist — primary covers all UI)
- **Additional Specialists**: (n/a in this config — other-engine agent removed) (GDExtension / native C++ bindings only)
...
| Game code (.gd files) | (n/a in this config — other-engine agent removed) |
| Game code (.cs files) | (n/a in this config — other-engine agent removed) |
| Cross-language boundary decisions | (n/a in this config — other-engine agent removed) |
| Shader / material files (.gdshader, VisualShader) | (n/a in this config — other-engine agent removed) |
| UI / screen files (Control nodes, CanvasLayer) | (n/a in this config — other-engine agent removed) |
| Scene / prefab / level files (.tscn, .tres) | (n/a in this config — other-engine agent removed) |
| Project config (.csproj, NuGet) | (n/a in this config — other-engine agent removed) |
