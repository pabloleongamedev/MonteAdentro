...
- **Tier 1 (Opus)**: Directors who make high-level decisions
  - `game-designer` -- vision and creative conflict resolution
  - `architect` -- architecture and technology decisions
  - `producer` -- scheduling, coordination, and risk management

- **Tier 2 (Sonnet)**: Department leads who own their domain
  - `game-designer`, `architect`, `art-audio`, `art-audio`,
    `narrative-writer`, `qa-engineer`, `producer`, `producer`
...
| Create a shader | `unity-specialist` |
| Write dialogue | `narrative-writer` |
| Plan the next sprint | `producer` |
| Review code quality | `architect` |
| Write test cases | `qa-engineer` |
| Design a level | `level-designer` |
| Fix a performance problem | `qa-engineer` |
| Set up CI/CD | `tools-devops` |
| Design a loot table | `game-designer` |
| Resolve a creative conflict | `game-designer` |
| Make an architecture decision | `architect` |
| Manage a release | `producer` |
| Prepare strings for translation | `producer` |
| Test a mechanic idea quickly | `gameplay-programmer` |
| Review code for security issues | `tools-devops` |
| Check accessibility compliance | `qa-engineer` |
| Get Unreal Engine advice | `(n/a in this config — other-engine agent removed)` |
| Get Unity advice | `unity-specialist` |
| Get Godot advice | `(n/a in this config — other-engine agent removed)` |
| Design GAS abilities/effects | `(n/a in this config — other-engine agent removed)` |
| Define BP/C++ boundaries | `(n/a in this config — other-engine agent removed)` |
| Implement UE replication | `(n/a in this config — other-engine agent removed)` |
| Build UMG/CommonUI widgets | `(n/a in this config — other-engine agent removed)` |
| Design DOTS/ECS architecture | `unity-specialist` |
| Write Unity shaders/VFX | `unity-specialist` |
| Manage Addressable assets | `unity-specialist` |
| Build UI Toolkit/UGUI screens | `unity-specialist` |
| Write idiomatic GDScript | `(n/a in this config — other-engine agent removed)` |
| Write Godot C# code | `(n/a in this config — other-engine agent removed)` |
| Create Godot shaders | `(n/a in this config — other-engine agent removed)` |
| Build GDExtension modules | `(n/a in this config — other-engine agent removed)` |
| Plan live events and seasons | `level-designer` |
...
2. **Write the Game Pillars** — delegate to `game-designer`
