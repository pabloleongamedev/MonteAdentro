...
```
Spawn `game-designer` via Task using gate **CD-PILLARS** from
...
Agent: `game-designer` | Model tier: Opus | Domain: Vision, pillars, player experience
...
documents are authored (team-narrative, design-system for story systems, narrative-writer
deliverables)
...
Agent: `architect` | Model tier: Opus | Domain: Architecture, engine risk, performance
...
Agent: `art-audio` | Model tier: Sonnet | Domain: Visual identity, art bible, visual production readiness
...
**Trigger**: After narrative-writer deliverables (dialogue, lore, item descriptions) are
...
1. game-designer  → gate CD-PHASE-GATE
2. architect → gate TD-PHASE-GATE
3. producer           → gate PR-PHASE-GATE
4. art-audio       → gate AD-PHASE-GATE
...
1. Assign a gate ID: `[DIRECTOR-PREFIX]-[DESCRIPTIVE-SLUG]`
   ...
   - Add new prefixes for new agents: `art-audio` → `AU-`, `game-designer` → `UX-`
